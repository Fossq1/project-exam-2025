    ...
    if (isEditing) {
      return (
        <article>
          ...
          <div className="py-2 flex gap-2 justify-center">
            <button className="border px-2 rounded-sm" onClick={saveButton}>
              Save
            </button>
            <button className="border px-2 rounded-sm" onClick={cancelButton}>
              Cancel
            </button>
          </div>
        </article>
      ...
      return (
        <div className="p-4">
          ...
          <h4 className="text-sm text-white/70">{athlete.gender}</h4>

          <div className="mt-2 flex items-center justify-between text-sm">
            <span
              className={
                athlete.purchaseStatus ? "text-red-500" : "text-emerald-400"
              }
            >
              {athlete.purchaseStatus ? "Purchased" : "Available"}
            </span>
            <span className="font-semibold text-white">
              {athlete.price.toLocaleString("nb-NO")}$
            </span>
          </div>
          ...
          {/* Separator between Buy and Admin buttons */}
